...
const transformCartItems = async (cartItems) => {
  ...
  const transformedItems = cartItems.map((item) => {
    ...
    const stockStatus = product.currentStock === 0 ? 'out_of_stock' : 
                       product.currentStock < product.optimalStock ? 'low_stock' : 'in_stock';
...
export const addToCart = async (req, res) => {
  try {
    ...
    if (quantity < 1 || quantity > 99) {
      return res.status(400).json({ message: '수량은 1개 이상 99개 이하여야 합니다.' });
    }
    ...
    if (existingItemIndex > -1) {
      ...
      if (newQuantity > 99) {
        return res.status(400).json({ message: '카트에 담을 수 있는 최대 수량은 99개입니다.' });
      }
      user.cart[existingItemIndex].quantity = newQuantity;
    } else {
      // 카트 최대 아이템 개수 제한 (20개)
      if (user.cart.length >= 20) {
        return res.status(400).json({ message: '카트에는 최대 20개의 상품만 담을 수 있습니다.' });
      }
      
      // 새 상품 추가 (재고 확인)
      if (quantity > product.currentStock) {
        return res.status(400).json({ message: `재고가 부족합니다. (현재 재고: ${product.currentStock}개)` });
      }
    ...
    res.json({
      success: true,
      message: existingItemIndex > -1 ? '수량이 증가되었습니다.' : '상품이 카트에 추가되었습니다.'
    });
...
export const updateCartItem = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(itemId)) {
      return res.status(400).json({ message: '유효하지 않은 카트 아이템 ID입니다.' });
    }
    ...
    if (quantity > 99) {
      return res.status(400).json({ message: '수량은 99개를 초과할 수 없습니다.' });
    }
    ...
    if (itemIndex === -1) {
      return res.status(404).json({ message: '카트 아이템을 찾을 수 없습니다.' });
    }
...
export const clearCart = async (req, res) => {
  try {
    ...
    console.log(`사용자 ${userId}의 카트에서 ${itemCount}개 아이템이 제거되었습니다.`);
