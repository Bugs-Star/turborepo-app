...
/**
 * Redis 스트림에서 메시지를 삭제
 * @param {Array<string>} ids - 삭제할 메시지 ID 배열
 */
export async function deleteMessages(ids) {
  if (!ids || ids.length === 0) return;
  const deleted = await redis.xdel(REDIS_STREAM_KEY, ...ids);
  console.log(`[Redis] Deleted ${deleted} messages`);
}

// --- 새로 추가: 스트림 길이 제한(XTRIM) ---
/**
 * Redis 스트림 길이 제한
 * @param {number} maxLen - 최대 메시지 개수
 */
export async function trimStream(maxLen = 1000) {
  if (!maxLen) return;
  await redis.xtrim(REDIS_STREAM_KEY, "MAXLEN", "~", maxLen);
  console.log(`[Redis] Stream trimmed to ~${maxLen} messages`);
}
