...
export function periodToRange(p: PeriodParams): DateRange {
  ...
  if (p.periodType === "weekly") {
    ...
    if (p.month) {
      ...
      const baseMondayUTC = startOfWeekKST(monthStartUTC); // 그 달 1일이 속한 주의 월요일(KST 기준)

      const weekStartUTC = addDays(baseMondayUTC, (w - 1) * 7); // 해당 주 시작(UTC)
      const weekEndUTC = addDays(weekStartUTC, 7); // 다음주 월요일(UTC)
      ...
      return { from: toYMD(clippedFromUTC), to: toYMD(clippedToUTC) }; // 항상 [from, to) 형태로 사용 권장
    }
